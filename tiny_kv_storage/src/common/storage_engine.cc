...
/* CacheMemoryStorage */
/************************************************************************/
CacheMemoryStorage::CacheMemoryStorage(size_t cache_capacity)
    : cache_(cache_capacity) {}

bool CacheMemoryStorage::Put(const std::string &key, const std::string &value) {
  std::lock_guard<std::mutex> lock(mutex_);
  data_[key] = value;
...
std::optional<std::string> CacheMemoryStorage::Get(const std::string &key) {
  auto cache_value = cache_.Get(key);
  if (cache_value) {
    return cache_value;
  }
...
bool CacheMemoryStorage::Delete(const std::string &key) {
  std::lock_guard<std::mutex> lock(mutex_);
  auto it = data_.find(key);
  if (it != data_.end()) {
    data_.erase(it);
...
KVMap CacheMemoryStorage::GetAllEntries() {
  std::lock_guard<std::mutex> lock(mutex_);
  return data_;
}
